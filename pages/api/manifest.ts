...
export default async function manifestEndpoint(req: NextApiRequest, res: NextApiResponse) {
    ...
    const protocolVersionMaybeArray: any = req.headers['expo-protocol-version'];
    ...
    const runtimeVersion =
        req.headers['expo-runtime-version'] ?? req.query['runtime-version'] ?? '1';
    ...
    try {
        ...
        console.log('error::::updateBundlePath = await getLatestUpdateB');
        console.log(error);
        res.statusCode = 404;
        res.json({
            error: error.message,
    ...
    console.log(`Serving manifest for ${runtimeVersion} on ${platform}...`, updateBundlePath);
    ...
    try {
        try {
            ...
            // 888888
            console.error('777777', maybeNoUpdateAvailableError);
            if (maybeNoUpdateAvailableError instanceof NoUpdateAvailableError) {
                console.error('777777-000000', maybeNoUpdateAvailableError);
                await putNoUpdateAvailableInResponseAsync(req, res, protocolVersion);
                return;
            }
            console.error('777777-111111', maybeNoUpdateAvailableError);
            throw maybeNoUpdateAvailableError;
        }
    } catch (error) {
        // 888888
        console.log('888888', updateBundlePath);
        console.error('888888-0000000', error);
        res.statusCode = 404;
        res.json({
            error: error.message,
            protocolVersionMaybeArray,
