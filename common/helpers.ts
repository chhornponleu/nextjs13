...
export async function getLatestUpdateBundlePathForRuntimeVersionAsync(runtimeVersion: string) {
    const updatesDirectoryForRuntimeVersion = process.cwd() + `/updates/${runtimeVersion}`;
...
export async function getAssetMetadataAsync(arg: GetAssetMetadataArg) {
    ...
    const baseUrl = process.env.VERCEL_URL || process.env.DEPLOY_URL;
    ...
    return {
        ...
        contentType,
        url: `${baseUrl}/api/assets?asset=${assetFilePath}&runtimeVersion=${arg.runtimeVersion}&platform=${arg.platform}`,
    };
}
...
export async function getExpoConfigAsync({
    ...
    try {
        ...
        console.log('pathhhhhh', path.resolve(expoConfigPath));
